...
The endpoint hostname for S3 storage. Omit or use `s3.amazonaws.com` for AWS S3.

#### `S3_REGION`

Example: `us-west-1`

The region for AWS S3. Unused for MinIO.

#### `S3_PORT`

Example: `443`, `9000`

The port S3 storage is running on. Omit or use `443` for AWS S3.

#### `S3_USE_SSL`

- Default: `false`

Whether to use SSL for S3 storage connections. Omit or use `true` for AWS S3.
